...
func (s *InvoiceService) CancelInvoice(ctx context.Context, id, orgID uuid.UUID, reason string) error {
	...
	// are returned via the separate refund flow. CancelInvoiceTx locks the
	// invoice, rejects lunas/already-cancelled, and builds the GL reversal
	// event from a fresh read under that lock (not a stale snapshot here).
	return s.repo.CancelInvoiceTx(ctx, id, orgID, reason)
}
